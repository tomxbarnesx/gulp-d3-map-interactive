...
let heatMaps = document.getElementsByClassName("heat-map-image");
let yearHeader = document.getElementById("year-header");
let activeMap = heatMaps.item(59);
	  ...
	  "features": [
	    {
	      ...
	      "properties": {
	      	"risk": "Floods",
	      	"popupContent": "The eastern seaboard faces a significantly larger threat from coastal flooding than does the Western seaboard. Increasingly turbulent Noreasters will exacerbate severe flooding as global temperatures increase.",
	      	"show_on_map": true,
	        "stroke": "#555555",
	        "stroke-width": 1,
	        "stroke-opacity": 1,
	  ...
	  "features": [
	    {
	      ...
	      "properties": {
	      	"risk": "Earthquakes",
	      	"popupContent": "Earthquakes will also become more common as invasive practices, like fracking and shale drilling to reach non-renawables, continue in techtonic regions. Additionally, drought and water table depletion may contribute to increased earthquake risk.",
	      	"show_on_map": true,
	        "stroke": "#555555",
	        "stroke-width": 1,
	        "stroke-opacity": 1,
...
function iframeCreate() {
    ...
    iFrame.src = "https://en.wikipedia.org/wiki/Quinta_da_Regaleira";
    iFrame.classList.add("iframe-master");
    detourContainer.appendChild(iFrame);
    
    //Load event complete, remove spinner
...
function handleYearChange(index){
	...
	activeMap.style.visibility = "hidden"
	newActive = heatMaps.item(index)
	newActive.style.visibility = "visible";
	activeMap = newActive;
	yearHeader.textContent = 1960 + parseInt(index);
	update(activeDataset, index);
}
